    ...
    public List<QuestionConfig> getQuestionConfigs() {
        ...
        if (ids.length != folders.length || ids.length != testers.length || ids.length != maxScores.length) {
            throw new IllegalStateException(
                    "Question configuration arrays must all be the same length. " +
                            "Got: list=" + ids.length + ", folders=" + folders.length +
                            ", testers=" + testers.length + ", maxScores=" + maxScores.length);
        }

        List<QuestionConfig> configs = new ArrayList<>();
        for (int i = 0; i < ids.length; i++) {
            configs.add(new QuestionConfig(
                    ids[i].trim(),
                    folders[i].trim(),
                    testers[i].trim(),
