...
public class FileUtils {
	...
	public static List<Path> findFilesByExtension(Path dir, String extension) throws IOException {
		...
		try (Stream<Path> stream = Files.list(dir)) {
			stream.filter(p -> Files.isRegularFile(p) && p.toString().endsWith("." + extension)).forEach(result::add);
		}
		return result;
	}
	...
	public static Path findFile(Path dir, String fileName) throws IOException {
		try (Stream<Path> stream = Files.walk(dir)) {
			return stream.filter(p -> Files.isRegularFile(p) && p.getFileName().toString().equals(fileName)).findFirst()
					.orElse(null);
		}
	}
	...
	public static Path findDirectory(Path root, String dirName) throws IOException {
		try (Stream<Path> stream = Files.walk(root)) {
			return stream.filter(p -> Files.isDirectory(p) && p.getFileName().toString().equals(dirName)).findFirst()
					.orElse(null);
		}
	}
